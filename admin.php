...
<div style="background-color:#130052; padding-top: 20px; padding-bottom: 20px;;"; id="admin" ><h1 style="text-align:center; color:white" >Admin Page</h1></div>
...
<table id="customers"  class="justify-item-center text-center" style="margin-top: 5rem;">
 ...
 $x = 1; //counter variable for user counting
  ...
  { $row = mysqli_fetch_assoc($result); //the fatched data will be stored into $run variable
    ...
    <td><h4><?php echo $row['fname'];?></h4></td>  <!-- this shows the first name of the user -->
    <td><h4><?php echo $row['lname'];?></h4></td>  <!-- this shows the Last name of the user -->
   
    <!-- by clicking on email id admin will redirect to gmail page -->
    <td><h4><a href="https://mail.google.com/mail/?view=cm&fs=1&to=<?php echo $row['email'];?>" ><?php echo $row['email'];?></a></h4></td>
    </tr>
    <?php
    $x++;
  }
